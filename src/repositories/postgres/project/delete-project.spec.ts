...
import { PostgresDeleteProjectRepository } from "./delete-project"
...
describe("Postgres Delete Project Repository", () => {
  ...
  const makeSut = () => {
    const sut = new PostgresDeleteProjectRepository()
  ...
  it("should delete project on database", async () => {
    const { sut } = makeSut()
    ...
    const result = await sut.execute(
      createProjectParams.id,
      createUserParams.id,
    )

    expect(result).not.toBeUndefined()
    expect(result.id).toBe(createProjectParams.id)
